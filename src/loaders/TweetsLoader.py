...
class TweetsLoader:
    def __init__(self, spark: SparkSession) -> None:
        self.spark = spark
        self.data_dir: str = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../..', 'data')
    ...
    def load_covid_tweets(self) -> DataFrame:
        file_path: str = os.path.join(self.data_dir, 'covid19_tweets.csv')
        return (self.spark.read.csv(file_path, header=True, schema=tweet_schema)
                .na.drop())

    def load_grammy_tweets(self) -> DataFrame:
        file_path: str = os.path.join(self.data_dir, 'GRAMMYs_tweets.csv')
        return (self.spark.read.csv(file_path, header=True, schema=tweet_schema)
                .na.drop())

    def load_financial_tweets(self) -> DataFrame:
        file_path: str = os.path.join(self.data_dir, 'financial.csv')
